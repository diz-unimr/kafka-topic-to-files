...
import de.unimarburg.diz.kafkatopic2files.config.AppFhirConfig;
import de.unimarburg.diz.kafkatopic2files.config.GIcsConfigProperties;
...
@ConditionalOnProperty(name = "app.consent.enabled", havingValue = "true")
@Service
public class GicsConsentService implements ICheckConsent {
  ...
  public GicsConsentService(
    ...
    httpHeader =
        buildHeader(gIcsConfigProperties.getUsername(), gIcsConfigProperties.getPassword());
  ...
  public String getGicsUri() {
    if (url == null) {
      ...
      url =
          UriComponentsBuilder.fromUriString(gIcsBaseUri).path(IS_CONSENTED_ENDPOINT).toUriString();
    }
    return url;
  }
  ...
  public static Parameters getIsConsentedParam(
      GIcsConfigProperties configProperties, String personIdentifierValue) {
    ...
    result.addParameter(
        new ParametersParameterComponent()
            ...
            .setValue(
                new Coding()
                    .setCode(configProperties.getPolicyCode())
    ...
    var config =
        new ParametersParameterComponent()
            .setName("de/unimarburg/diz/kafkatopic2files/config")
            .addPart(
                new ParametersParameterComponent()
  ...
  public TtpConsentStatus getTtpConsentStatus(String personIdentifierValue) {
    var parameter =
        GicsConsentService.getIsConsentedParam(gIcsConfigProperties, personIdentifierValue);
